<template>
	<div class="container">
		<article>
			...
			<div v-if="article.image && article.image.src" class="flex article-img-container">
				<img :src="`/posts/${article.image.src}`" class="article-img" />
			</div>
			<div class="flex flex-wrap article-tags-container" v-if="article.tags.length">
				<div>Catégories:</div>
				<div v-for="(tag, indexTag) in article.tags.split(';')" :key="`indexTag-${indexTag}`" class="flex article-tag">
					{{ tag }}
				</div>
			</div>

			<div v-if="article.author" class="flex author-container">
				<img :src="`/author/${article.author.avatar}`" alt="auteur" class="author-logo" />
	...
	export default {
		async asyncData({ $content, params, error }) {
			try {
				...
				const [prev, next] = await $content("posts").only(["title", "path"]).sortBy("createdAt").where({ archived: false }).surround(params.post).fetch();
		...
		jsonld() {
			const res = {
				...
				mainEntityOfPage: {
					"@type": "WebPage",
					"@id": this.$config.hostname + this.article.path,
				},
      ...
      if (this.article?.image?.src) {
        res.image =  [`${this.$config.hostname}/posts/${this.article.image.src}`]
      }

      if (this.article.updatedAt) {
        res.dateModified = this.article.updatedAt
      }
			return res;
		},
